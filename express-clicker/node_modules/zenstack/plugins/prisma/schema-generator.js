...
`;
    ...
    generateDelegateRelationForBase(model, decl) {
        ...
        concreteModels.forEach((concrete) => {
            const auxName = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${this.truncate((0, lower_case_first_1.lowerCaseFirst)(concrete.name))}`;
    ...
    generateDelegateRelationForConcrete(model, concreteDecl) {
        ...
        baseModels.forEach((base) => {
            ...
            // add relation fields
            const relationField = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${this.truncate((0, lower_case_first_1.lowerCaseFirst)(base.name))}`;
    ...
    expandPolymorphicRelations(model, dataModel) {
        ...
        dataModel.fields.forEach((field) => {
            ...
            concreteModels.forEach((concrete) => {
                ...
                const auxRelationField = model.addField(`${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${this.truncate(auxRelationName)}`, new prisma_builder_1.ModelFieldType(concrete.name, field.type.array, field.type.optional));
    ...
    replicateForeignKey(model, dataModel, concreteModel, origForeignKey) {
        ...
        addedFkField.name = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${this.truncate(addedFkFieldName)}`;
    ...
    nameRelationsInheritedFromDelegate(model, decl) {
        ...
        decl.fields.forEach((f) => {
            ...
            const relName = `${runtime_1.DELEGATE_AUX_RELATION_PREFIX}_${this.truncate(name)}`;
            if (relAttr) {
                const nameArg = (0, sdk_1.getAttributeArg)(relAttr, 'name');
                if (!nameArg) {
