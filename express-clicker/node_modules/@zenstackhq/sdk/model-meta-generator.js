...
function generateModelMetadata(dataModels, sourceFile, writer, options) {
    writer.block(() => {
...
function writeModels(sourceFile, writer, dataModels, options) {
    writer.write('models:');
    writer.block(() => {
...
function writeAuthModel(writer, dataModels) {
...
function writeFields(sourceFile, writer, model, options) {
    writer.write('fields:');
    writer.block(() => {
        var _a;
        ...
        isRelationOwner: true,`);
            ...
            if ((0, _1.isForeignKeyField)(f)) {
                writer.write(`
        isForeignKey: true,`);
                const relationField = (0, _1.getRelationField)(f);
                if (relationField) {
                    writer.write(`
        ...
        foreignKeyMapping: ${JSON.stringify(fkMapping)},`);
            ...
            const inheritedFromDelegate = (0, _1.getInheritedFromDelegate)(f);
            if (inheritedFromDelegate && !(0, _1.isIdField)(f)) {
                writer.write(`
...
function getRelationName(field) {
    const relAttr = (0, _1.getAttribute)(field, '@relation');
    if (!relAttr) {
        return undefined;
    }
    return (0, _1.getAttributeArgLiteral)(relAttr, 'name');
}
...
function generateDefaultValueProvider(field, sourceFile) {
    var _a;
